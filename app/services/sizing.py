...
def position_size_vol_scaled(
    price: float, atr_proxy: float | None, fallback_fraction: float, cash: float
) -> tuple[float, str]:
    if not atr_proxy:
        return (
            position_size_fixed_fraction(cash, fallback_fraction, price),
            "fallback_fixed_fraction",
        )
